...
def _merge_and_overwrite_lora(save_directory, filename, lora_weights, output_dtype, model_class_name, base_model_is_quantized=False, quant_type=None):
    ...
    with safe_open(filename_original, framework="pt", device="cpu") as file:
        ...
        # Pre-calculate tensor info for look-ahead
        tensor_info_list = []
        for idx, key in enumerate(safetensor_keys[:100]):  # Sample first 100 for look-ahead
            if key in processed_mxfp4_keys:
                continue

            try:
                W_sample = file.get_tensor(key)
                tensor_info_list.append({'key': key, 'tensor': W_sample, 'index': idx})
            except:
                continue
        ...
        for idx, key in enumerate(safetensor_keys):
            ...
            # Calculate optimal batch size based on upcoming tensors
            remaining_tensor_info = tensor_info_list[tensor_info_index + 1:]
            optimal_batch_size = calculate_memory_aware_batch_size(
                current_batch_tensors,
                remaining_tensor_info,
                converted_lora_weights,
                output_dtype,
                safety_factor=0.85  # More aggressive - you have plenty of RAM
            )
            ...
            should_process_batch = (
                ...
                current_batch_memory > stats['cpu']['available'] * 0.85  # Safety check
            )
